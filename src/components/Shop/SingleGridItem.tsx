...
import { FiImage } from "react-icons/fi"; // Ikon untuk gambar placeholder

const SingleGridItem = ({ item }: { item: Product }) => {
  const { openModal } = useModalContext();
  const dispatch = useDispatch<AppDispatch>();

  // Cek apakah ada URL gambar yang valid di dalam array
  ...
  return (
    <div className="group">
      {/* === DIKEMBALIKAN KE DESAIN ASLI ANDA (bg-white) === */}
      <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-white shadow-1 min-h-[270px] mb-4">
        ...
        {imageUrl ? (
          <Image 
            ...
            width={250} 
            height={250}
            className="object-contain w-full h-full p-4 group-hover:scale-105 transition-transform duration-300 ease-in-out"
          />
        ) : (
          // Tampilan pengganti jika tidak ada gambar
      ...
      <div className="flex items-center gap-2.5 mb-2">
        <div className="flex items-center gap-1">
          {/* Anda bisa membuat rating dinamis di sini jika ada datanya */}
      ...
      <span className="flex items-center gap-2 font-medium text-lg">
        <span className="text-dark">Rp.{item.discountedPrice?.toLocaleString('id-ID') || item.price.toLocaleString('id-ID')}</span>
