...
const SingleItem = ({ item }: { item: CartItem }) => {
  ...
  // 2. Tentukan harga mana yang akan ditampilkan
  // Jika ada harga diskon, gunakan itu. Jika tidak, gunakan harga normal.
  const displayPrice = item.discountedPrice ?? item.price;

  // === AKHIR PERBAIKAN LOGIKA ===

  return (
    // Desain Anda dipertahankan 100%
    <div className="flex items-center justify-between gap-5">
      <div className="w-full flex items-center gap-6">
        <div className="flex items-center justify-center rounded-[10px] bg-gray-3 dark:bg-gray-700 max-w-[90px] w-full h-22.5">
          {imageUrl ? (
        ...
        <div>
          <h3 className="font-medium text-dark dark:text-white mb-1 ease-out duration-200 hover:text-blue">
            <Link href={`/shop-details/${item.id}`}> {item.title} </Link>
          </h3>
          {/* Tampilkan harga yang sudah benar dan diformat */}
          <p className="text-custom-sm text-gray-600 dark:text-gray-400">
            Price: Rp.{displayPrice?.toLocaleString('id-ID')}
          </p>
        </div>
      </div>
      ...
      <button
        ...
        className="flex-shrink-0 flex items-center justify-center rounded-lg max-w-[38px] w-full h-9.5 bg-gray-2 border border-gray-3 text-dark ease-out duration-200 hover:bg-red-light-6 hover:border-red-light-4 hover:text-red"
      >
