...
import { FiImage } from "react-icons/fi"; // Impor ikon untuk placeholder

const SingleItem = ({ item }: { item: CartItem }) => {
  const [quantity, setQuantity] = useState(item.quantity);
  const dispatch = useDispatch<AppDispatch>();

  // Sinkronkan state lokal dengan Redux jika berubah dari sumber lain
  useEffect(() => {
  ...
  // 1. Ambil URL gambar dengan aman.
  // Kode asli Anda menggunakan item.imgs?.thumbnails[0], kita ganti ke imagePreviews
  const imageUrl = item.imagePreviews && item.imagePreviews.length > 0 ? item.imagePreviews[0] : null;

  // 2. Tentukan harga yang akan ditampilkan (harga diskon, atau harga normal jika tidak ada)
  const displayPrice = item.discountedPrice ?? item.price;
  
  // 3. Hitung subtotal dengan aman
  const subtotal = displayPrice * quantity;

  // === AKHIR PERBAIKAN LOGIKA ===

  return (
    // Seluruh JSX dan className Anda di bawah ini tidak diubah sama sekali
    <div className="flex items-center border-t border-gray-300 dark:border-gray-700 py-5 px-7.5">
      <div className="min-w-[400px]">
        <div className="flex items-center justify-between gap-5">
          <div className="w-full flex items-center gap-5.5">
            <div className="flex items-center justify-center rounded-[5px] bg-gray-200 dark:bg-gray-700 max-w-[80px] w-full h-17.5">
              {/* Gunakan imageUrl yang sudah aman, dengan fallback */}
              {imageUrl ? (
            ...
            <div>
              <h3 className="text-dark dark:text-white ease-out duration-200 hover:text-blue-600">
                <a href="#"> {item.title} </a>
              </h3>
            </div>
          </div>
      ...
      <div className="min-w-[180px]">
        {/* Tampilkan harga yang sudah benar dan diformat */}
        <p className="text-dark dark:text-white">Rp{displayPrice.toLocaleString('id-ID')}</p>
      </div>
      ...
      <div className="min-w-[200px]">
        {/* Tampilkan subtotal yang sudah benar dan diformat */}
        <p className="text-dark dark:text-white">Rp{subtotal.toLocaleString('id-ID')}</p>
      </div>
